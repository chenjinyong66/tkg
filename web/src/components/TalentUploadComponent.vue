  ...
  >
    ...
    <div class="add-files-content">
      ...
      <div class="upload" v-if="uploadMode === 'file'">
        ...
        <!-- 新增ZIP专用上传组件 -->
        <div class="zip-upload-section" v-if="showZipUpload">
          <h3>ZIP文件专用上传</h3>
          <ZipUploadComponent
              :db-id="databaseId"
...
// 导入ZIP上传组件
import ZipUploadComponent from '@/components/TalentUploadComponent.vue';
import {
  FileOutlined,
  LinkOutlined,
  SettingOutlined,
...
// 处理ZIP上传成功的回调
const handleZipUploadSuccess = (result) => {
  message.success('ZIP文件上传处理成功');
  // 可以在这里触发其他操作，比如刷新文件列表等
  console.log('ZIP上传结果:', result);
};
</script>
