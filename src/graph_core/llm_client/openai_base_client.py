...
DEFAULT_SMALL_MODEL = os.getenv('OPENAI_SMALL_MODEL', 'gpt-4.1-nano')
...
class BaseOpenAIClient(LLMClient):
    ...
    def _handle_structured_response(self, response: Any) -> dict[str, Any]:
        ...
        # Check if response has output_text attribute (for structured responses)
        if hasattr(response, 'output_text'):
            response_object = response.output_text
        else:
            # If response is already a parsed dict or other format, handle accordingly
            if isinstance(response, dict):
                return response
            elif hasattr(response, 'choices') and response.choices:
                # Handle case where response is a raw API response
    ...
    async def _generate_response(
        ...
        try:
            if response_model:
                logger.debug(f'测试LLM request,: {openai_messages}')
                ...
                logger.warning(f'if response_model 测试dLLM response: {response}')
                return self._handle_structured_response(response)
            else:
                logger.debug(f'测试LLM request,: {openai_messages}')
                logger.debug(f'测试LLM model,: {model}')
                ...
                logger.warning(f'else response_model测试dLLM response: {response}')
    ...
    async def generate_response(
        ...
        with self.tracer.start_span('llm.generate') as span:
            ...
            while retry_count <= self.MAX_RETRIES:
                try:
                    logger.warning(f'测试LLM request,: {messages},response_model: {response_model}')
                    response = await self._generate_response(
                        messages, response_model, max_tokens, model_size
                    )
                    logger.debug(f'测试LLM response: {response}')
                    return response
